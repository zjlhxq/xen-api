    ...
    let attach context ~dbg ~dp ~sr ~vdi ~read_write =
      ...
      let backend = attach2 context ~dbg ~dp ~sr ~vdi ~read_write in
			(* VDI.attach2 should be used instead, VDI.attach is only kept for
				 ...
				 we return from here. *)
      let (xendisks, blockdevs, files, nbds) = Attach_helpers.implementations_of_backend backend in
      (* TODO what to do here? *)
      let response ?(backend_kind="vbd3") params =
        (* TODO We've thrown o_direct away from the SMAPIv1 info during the conversion to SMAPIv3 attach info *)
        { params; xenstore_data = ["backend-kind", backend_kind]; o_direct = true; o_direct_reason = "" }
      in
      match files, blockdevs, xendisks, nbds with
      | file::_, _, _, _ -> response file.Storage_interface.path
      | _, blockdev::_, _, _ -> response blockdev.Storage_interface.path
      | _, _, xendisk::_, _ ->
        let backend_kind = match xendisk.Storage_interface.backend_type with
          ...
          | x -> x
        in
        response ~backend_kind xendisk.Storage_interface.params
      | _ -> raise (Storage_interface.Backend_error (Api_errors.internal_error, ["No BlockDev or XenDisk implementation in Datapath.attach response: " ^ (rpc_of_backend backend |> Jsonrpc.to_string)]))
